...
%TODO: Notion of storage elasticity; expand on its use, I/O throughput and such.
...
\begin{itemize}
  \item Globally implemented clouds rely heavily on internet backbone resulting
  in a non-uniform and variable network characteristics which application
  deployments must take into account. Storage solutions can mitigate resulting
  variabilities to some extent by techniques such as caching, replication and
  prediction.

  \item Applications with small to medium immediate storage requirements can be
  run effectively by aggregating the cloud node-local space with the help of
  storage solutions. These solutions almost always perform better compared to
  the dedicated object store provided by clouds such as S3 by Amazon.

  \item Storage solutions such as S3 are nonetheless important for large-scale
  data handling and archival purposes.

  \item Depending upon the application requirements, Swift can handle both
  implicit and explicit data motions in the cloud.
...
== Some Ideas ==
Following is a collection of ideas from comments, discussions, reviews:

-- Coherence
The paper is 3 parts that aren’t really connected, except thematically.
Coherence is lacking. Can we tie them together better through common
experiments, such as running some of the applications on the worst and best
bandwidth and latency cloud instances, to see what happens, and perhaps to run
the applications using worse filesystems as well, and showing the differences?
Can we identify/deduce cloud hotspots to place task and storage
servers/coordinators and see performance difference? Perhaps use a synthetic
workflow to tune data and compute. 

-- Deeper Understanding
A deeper understanding on runtime behavior of systems is missing. I think it
would be valuable to answer questions like: How is the data stored and
distributed once it enters the storage system? How much locality: how many
remote calls per MB? How well the tasks and data are keeping up? Likely data is
lagging behind tasks: by how much in each systems? How can we quantify and
improve this? Can we analyze logs from a workflow execution to answer these
questions? Transfer block size vs performance of storage systems? Separate I/O
and execution time. This will address a major comment by reviewers: insight
into results. Can we say a system is better than other in some circumstances?
eg. FusionFS is better under A and B while Mosa is better under X and Y.
Swift/T should be used when XYZ whereas T should be used when ABC.

-- Stress Test
How much stress can these system undergo in terms of handling large quantities
of data given enough physical space? How well are they suited to data-intensive
workflow style computations? Are they ready for BigData? How many cloud
instances can run them without significant faults or performance degradation?

-- Persistence
Can the data be persisted in the system beyond a single run? Beyond the
lifecycle of a VM? "Persistent aggregation of Storage Infrastructure", This is
an idea proposed by Mike a while ago. We push data to the /scratch or dev/shm
of a large system. We aggregate it via one of the storage solutions: Chirp,
Mosa, HDFS. We let it stay there for a longer term and allow users to do
multiple non-continuous analyses on that data over time.

-- Cost and Prediction
Comment about the savings in terms of monetary costs under different
conditions. Model the experiments to give a prediction of cost ahead of time
for experiments.

-- The cloud storage pyramid. Elaborate and expand. Could be quite informative.

-- Package and Release
Package the system as a reusable cloud image and release it for users. The
image would contain preinstalled, configured software tools (Swift T, K, Mosa,
Fusion, HDFS, code for analysis, logging etc.) and application examples that
others might find useful. Users can use any of the available combinations for
their situation and requirements. With this, make a case for reuse and
reproducibility of the work.

A scheme and related development/implementation to push subsequent updates to
software to the cloud image as part of its maintenance and upgradation.

-- Can this scenario be called RaaS? Resource as a service?

-- "Cloud provisioning from spot market", My recent work of cloud resource provisioning. A user can attach/detach cloud resources to a running application from spot market. This will also address the cost concern of cloud systems, as commented by a reviewer.
-- Provisioned IOPS? Find more about this.
-- network weather on the global backbone
-- Data management in the cloud is a hot research topic.

==Experiments Description==

Aim: To obtain a detailed understanding of data movements and metadata changes over a set of cloud instances for a given storage system working in conjunction with a workflow encoded application execution.

Statement: Run a workflow using Swift with S3, Mosa and FusionFS each over a set number of cloud instances for a set workflow I/O profile.

1. Workflow: Skeleton generated synthetic workflow (essentially a DAG). 5
stages, one branch. Each stage has between 10 and 100 tasks with a total of 500
computationally medium intensive tasks. Stages produce and consume data
arbitrarily. Approximately between 100 and 1000 MB of I/O per workflow run.

2. Swift: Run with SwiftK with explicit data movement over locally hosted data
and with SwiftT with implicit data movement over storage system hosted data.
Logging enabled.

3. S3: Run S3 in remote and mounted with fuse mode. Enable full Fuse logging.

4. Mosa and FusionFS: Run'em in full logging mode with default configuration.

5. Cloud instances: Run over 20 and 40 cloud instances (2 regions) of type
medium : 2 core 10G+ memory instances. Need to work out the cost.


Assuming a hypothetical commandline controlling experiment parameters, we will
have the following 8 sets of experiments with 5 runs in each resulting in a
grand total of 40 runs.

runexp swiftK --datasize=100m  --cloudinstances=20 --storagesystem=none #baseline distributed workflow execution, classic Swift, no storage system, job and data distribution is done by Swift.
runexp swiftK --datasize=200m  --cloudinstances=20 --storagesystem=none 
runexp swiftK --datasize=500m  --cloudinstances=20 --storagesystem=none
runexp swiftK --datasize=750m  --cloudinstances=20 --storagesystem=none
runexp swiftK --datasize=1000m --cloudinstances=20 --storagesystem=none

runexp swiftK --datasize=100m  --cloudinstances=40 --storagesystem=none #cloud instances across two regions with explicit staging
runexp swiftK --datasize=200m  --cloudinstances=40 --storagesystem=none
runexp swiftK --datasize=500m  --cloudinstances=40 --storagesystem=none
runexp swiftK --datasize=750m  --cloudinstances=40 --storagesystem=none
runexp swiftK --datasize=1000m --cloudinstances=40 --storagesystem=none


runexp swiftT --datasize=100m  --cloudinstances=20 --storagesystem=s3fs #S3 as mounted FS via FUSE, one region, implicit staging
runexp swiftT --datasize=200m  --cloudinstances=20 --storagesystem=s3fs
runexp swiftT --datasize=500m  --cloudinstances=20 --storagesystem=s3fs
runexp swiftT --datasize=750m  --cloudinstances=20 --storagesystem=s3fs
runexp swiftT --datasize=1000m --cloudinstances=20 --storagesystem=s3fs

runexp swiftT --datasize=100m  --cloudinstances=40 --storagesystem=s3fs #S3 as mounted FS via FUSE, two regions, implicit staging
runexp swiftT --datasize=200m  --cloudinstances=40 --storagesystem=s3fs
runexp swiftT --datasize=500m  --cloudinstances=40 --storagesystem=s3fs
runexp swiftT --datasize=750m  --cloudinstances=40 --storagesystem=s3fs
runexp swiftT --datasize=1000m --cloudinstances=40 --storagesystem=s3fs


runexp swiftT --datasize=100m  --cloudinstances=20 --storagesystem=mosa #same as above, except with mosa
runexp swiftT --datasize=200m  --cloudinstances=20 --storagesystem=mosa
runexp swiftT --datasize=500m  --cloudinstances=20 --storagesystem=mosa
runexp swiftT --datasize=750m  --cloudinstances=20 --storagesystem=mosa
runexp swiftT --datasize=1000m --cloudinstances=20 --storagesystem=mosa

runexp swiftT --datasize=100m  --cloudinstances=40 --storagesystem=mosa
runexp swiftT --datasize=200m  --cloudinstances=40 --storagesystem=mosa
runexp swiftT --datasize=500m  --cloudinstances=40 --storagesystem=mosa
runexp swiftT --datasize=750m  --cloudinstances=40 --storagesystem=mosa
runexp swiftT --datasize=1000m --cloudinstances=40 --storagesystem=mosa


runexp swiftT --datasize=100m  --cloudinstances=20 --storagesystem=fusion #same as above, except with fusion
runexp swiftT --datasize=200m  --cloudinstances=20 --storagesystem=fusion
runexp swiftT --datasize=500m  --cloudinstances=20 --storagesystem=fusion
runexp swiftT --datasize=750m  --cloudinstances=20 --storagesystem=fusion
runexp swiftT --datasize=1000m --cloudinstances=20 --storagesystem=fusion

runexp swiftT --datasize=100m  --cloudinstances=40 --storagesystem=fusion
runexp swiftT --datasize=200m  --cloudinstances=40 --storagesystem=fusion
runexp swiftT --datasize=500m  --cloudinstances=40 --storagesystem=fusion
runexp swiftT --datasize=750m  --cloudinstances=40 --storagesystem=fusion
runexp swiftT --datasize=1000m --cloudinstances=40 --storagesystem=fusion

I propose the 10 runs corresponding to mosa and fusion be carried out by Hao
and Dongfang respectively. I can do the rest of the runs. I can work with Zhao
to build the synthetic workflow from his skeleton generation framework. Since
we will be working on identical cloud instances on same zones with servers etc
running within cloud, it nearly guarantees a uniform experimental environment
minimizing the chances of errors. We exchange log information after a first run
(pilot run) in each set and make sure we are in the same ballpark in terms of
